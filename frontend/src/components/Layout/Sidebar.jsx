...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-dark-100 border-r border-slate-200 dark:border-slate-800 flex flex-col h-screen fixed left-0 top-0 transition-all duration-300">
      <div className="p-6 flex items-center gap-3">
        <div className="w-10 h-10 bg-primary-600 rounded-xl flex items-center justify-center shadow-glow">
          <Zap size={24} className="text-white" />
        </div>
        <span className="font-display font-black text-xl text-slate-900 dark:text-white tracking-tighter">Ride For You</span>
      </div>
        ...
        {menuItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) => `
              ...
              ${isActive 
                ? 'bg-primary-600/10 text-primary-500 dark:text-primary-400 border border-primary-500/20' 
